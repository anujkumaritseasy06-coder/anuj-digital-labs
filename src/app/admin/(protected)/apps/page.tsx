...
export default function AppsListPage() {
  ...
  const handleTogglePublish = async (app: AdminApp) => {
    setTogglingId(app.id);
    const next = app.publishStatus === 'published' ? 'draft' : 'published';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">{apps.length} app{apps.length !== 1 ? 's' : ''} found</p>
        </div>
        <Link
          href="/admin/apps/new"
          id="apps-add-btn"
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="text-xs px-2 py-1 rounded-full"
                        style={{
                          background: app.releaseStatus === 'available' ? 'rgba(52,211,153,0.1)' : 'rgba(251,191,36,0.1)',
                          color: app.releaseStatus === 'available' ? '#34d399' : '#fbbf24',
                          border: `1px solid ${app.releaseStatus === 'available' ? 'rgba(52,211,153,0.25)' : 'rgba(251,191,36,0.25)'}`,
                        }}>
                        {app.releaseStatus === 'available' ? 'Available' : 'Coming Soon'}
                      </span>
                    </td>
                    <td className="px-6 py-4"><StatusBadge status={app.publishStatus} size="sm" /></td>
                    <td className="px-6 py-4 text-slate-500 text-center">{app.displayOrder}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {/* Toggle publish */}
                        <button
                          id={`toggle-publish-${app.id}`}
                          ...
                          style={app.publishStatus === 'published'
                        ...
                        >
                          {app.publishStatus === 'published' ? 'Unpublish' : 'Publish'}
                        </button>
                        {/* Edit */}
                        <Link
