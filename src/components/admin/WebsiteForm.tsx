...
import type { AdminWebsite, WebsiteCategory, PublishStatus } from '@/types/websites';
import ImageUploader from './ImageUploader';

const CATEGORIES: WebsiteCategory[] = [
  'Corporate', 'Business', 'Restaurant', 'Education', 'Healthcare', 'Portfolio', 
...
export default function WebsiteForm({ initial, mode }: WebsiteFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent, publishStatus?: PublishStatus) => {
    e.preventDefault();
    if (!form.name || !form.category || !form.bannerImage || !form.websiteUrl) {
      setError('Please fill in all required fields (Name, Category, Banner Image, Website URL).');
      return;
    }
    setError('');
    setSaving(true);
      ...
      <div className="p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-5" style={{ background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="md:col-span-2">
          <Field label="Website Name" required>
            <Input value={form.name} onChange={e => setForm({ name: e.target.value })} maxLength={100} placeholder="e.g. Acme Corp" />
          </Field>
        </div>
        ...
        <div className="md:col-span-2">
          <Field label="Banner Image" required hint="Used on the portfolio card.">
            <ImageUploader id="bannerImage" label="Upload Banner Image" value={form.bannerImage || ''} onChange={url => setForm({ bannerImage: url })} uploadHint="screenshot" />
          </Field>
        </div>

        <div className="md:col-span-2">
          <Field label="Website URL" required hint="The live link to open when clicked.">
        ...
        <div>
          <Field label="Sort Order" hint="Lower numbers appear first">
            <Input type="number" value={form.sortOrder} onChange={e => setForm({ sortOrder: parseInt(e.target.value) || 0 })} />
          </Field>
        </div>
